...
import { BookOpen, Grid3X3, HelpCircle, Info, Phone, Scissors, Sparkles, X } from 'lucide-react';
          ...
          <>
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-ink-800/60 dark:text-cream-100/60">
                  More
                </span>
                <button
                  type="button"
                  ...
                  className="grid h-8 w-8 place-items-center rounded-full bg-ink-800/5 transition-colors hover:bg-ink-800/10 dark:bg-cream-100/5 dark:hover:bg-cream-100/10"
                >
                  <X size={14} />
                </button>
              </div>
              <nav className="flex flex-col gap-1">
                {moreLinks.map(({ icon: Icon, label, href }) => (
                  <a
                    ...
                    className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink-800/80 transition-colors hover:bg-bronze-500/10 hover:text-bronze-500 dark:text-cream-100/80 dark:hover:text-bronze-400"
                  >
                    <Icon size={18} />
                    {label}
                  </a>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center justify-center gap-0.5 px-2 py-1 text-ink-800/70 transition-colors hover:text-bronze-500 active:text-bronze-500 dark:text-cream-100/70 dark:hover:text-bronze-400 dark:active:text-bronze-400"
            >
              <Icon size={20} />
